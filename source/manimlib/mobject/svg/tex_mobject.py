...
Contains all Mobjects related to TeX.
"""
...
class TexMobject(SingleStringTexMobject):
    ...
    def __init__(self, *tex_strings, **kwargs):
        ...
        self.substrings_to_isolate = [subs 
                                      for subs in self.substrings_to_isolate 
                                      if subs not in self.excluded_strings]
...
class TextMobject(TexMobject):
    """
    `manimlib.mobject.svg.tex_mobject.TextMobject`
    TextMobject
    ===========
    A special variant of `TexMobject` which writes in text mode.
...
class BulletedList(TextMobject):
    ...
    def fade_all_but(self, index_or_string, opacity=0.5):
        """
        Fades out all items other than `index_or_string`
        """
        arg = index_or_string
...
class Title(TextMobject):
    """
    `manimlib.mobject.svg.tex_mobject.Title`
    Title
    =====
    Writes a centered, underlined title.
