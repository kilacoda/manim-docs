...
class Scene(Container):
    ...
    This class contains lots of useful functions, though most of the time
    you'll be fine with only ``Scene.play()`` and ``Scene.wait()``
    ...
    * leave_progress_bars\n
    ...
    def construct(self):
        """
        The function looked for by manim while rendering a Scene.
        """
    ...
    def print_end_message(self):
        """
        The message one sees at the end of a render.
        """
    ...
    """
    Methods relating to the camera.
    """
    ...
    ###
    """
    Methods that act `en masse` on the Scene's Mobjects.
    """
    ...
    def remove(self, *mobjects):
        """
        To remove Mobjects in the reverse order they were ``Scene.add``ed.
        """
    ...
    def get_animation_time_progression(self, animations):
        """
        The method defining the ProgressBar you see on a render.
        """
    ...
    def handle_play_like_call(func):
        """
        A wrapper for implementing ``Scene.play()`` and ``Scene.wait()``
        """
    ...
    def play(self, *args, **kwargs):
        """
        Plays the animations (args) one after the other and associates
        the arguments (kwargs) following them.
        """
        if len(args) == 0:
    ...
    def wait(self, duration=DEFAULT_WAIT_TIME, stop_condition=None):
        """
        To insert a pause of ``duration`` seconds. Defaults to 1sec.
        """
    ...
    def get_indices(self, mob):
        for index in range(len(mob)):
            direction = UP if index == 0 or index%2 == 0 else DOWN

            self.play(Write(Text(f"{index}", size=0.19,opacity=0.3)\
                                .next_to(mob[index], direction)))
